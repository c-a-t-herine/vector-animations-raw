...
WIN_TITLE = "Victor Star Picker"
FIXED_WIDTH = 360
MAIN_IMAGE_PARAMETERS = [350.0, 540.0]
DOCKING_WINDOW = ['MacBlast', 'ChannelBoxLayerEditor', 'AttributeEditor']
JSON_FILE = os.path.join(os.path.dirname(__file__), "star_picker.json")
TOOLS_DIR_ENV_VAR = "ANKI_TOOLS"
UI_ICONS_DIR = os.path.join(os.getenv(TOOLS_DIR_ENV_VAR), "icons", "starPickerUI")
PICKER_BG_PNG = os.path.join(UI_ICONS_DIR, "star_picker_bg.png")
...
class StarPicker(QWidget):
    """
    Overall window, responsible for scrolling
    """
    def __init__(self, *args, **kwargs):
        super(StarPicker, self).__init__(*args, **kwargs)
...
def main():
    ...
    ui, dockWidget, _globalDockControl = Dock(StarPicker, winTitle=WIN_TITLE)
    return ui
